...
def create_customers(n):
    ...
    Channel_Onboarding = ['E-mail', 'In Person', 'In person - In Branch/Bank Office',
                          'In person - Offsite/Client Location', 'Mail', 'Online', 'Phone',
                          'Request for Proposal (RFP)'] + ['Not Applicable'] * 10
    # List for random Transaction flag
    Channel_Ongoing_Transactions = ['ATM', 'E-mail', 'Fax', 'Mail', 'Not Applicable', 'OTC Communication System',
                                    'Phone'] + ['Online'] * 4 + ['In Person'] * 31
    ...
    Use_Case = [1, 4, 7, 10, 13, 16, 19, 22, 25, 28, 31, 34, 39] * 4 + [2, 5, 8, 11, 14, 17, 20, 23, 26, 29, 32, 35,
                                                                        38] * 7 + [3, 6, 9, 12, 15, 18, 21, 24, 27, 30,
                                                                                   33, 36] * 65 + [37] * 73 + [40,
                                                                                                               41] * 2
    ...
    start = 10786147
    acct_list = []
    risk_flags = {'PEP': 'No', 'SAR': 'No', 'Clsd': 'No', 'high_risk': 'No', 'hr_rating': 'No', 'demarket': ''}
    liCSV = []
    for i in xrange(n):
        ...
        row.extend([names[0], mdl[0], ssn[0], names[1], mdl[1], ssn[1], names[2], mdl[2], ssn[2], printCC, CC_NO[0],
                    gen_data.create_company_name() + ' ' + tmp[1], gen_data.create_email(),
                    gen_data.create_job_title()])
        # Create Current Address
        zip = random.choice(zips.zip)
        addr = gen_data.create_city_state_zip([zip])
        # Create Previous address
        zip2 = random.choice(zips.zip)
        addr2 = gen_data.create_city_state_zip([zip2])
    ...
    with open('uber_cust.csv', 'w') as f1:
        ...
        writer.writerow(['ROWNUM'] + ['ACCOUNTID'] + ['ACCT_TYPE'] + ['NUM_CCS'] + ['NAME'] + ['M_NAME'] + ['SSN'] + [
            'AUTHORIZED_NAME2'] + ['M_NAME2'] + ['SSN2'] + \
                        ['AUTHORIZED_NAME3'] + ['M_NAME3'] + ['SSN3'] + ['AUTHORIZED_NAME4'] + ['M_NAME4'] + [
                            'SSN4'] + ['`NUMBER'] + ['CREDITCARDTYPE'] + ['EMPLOYER'] + ['CUSTEMAIL'] + \
...
results = pool.map(createSSNs, (
    numUniSSNs / 10, numUniSSNs / 10, numUniSSNs / 10, numUniSSNs / 10, numUniSSNs / 10, numUniSSNs / 10,
    numUniSSNs / 10,
    numUniSSNs / 10, numUniSSNs / 10, ((numUniSSNs / 10) + remainder)))
liSSNMaster = results[0] + results[1] + results[2] + results[3] + results[4] + results[5] + results[6] + results[7] + \
              results[8] + results[9]
...
if len(liSSNMaster) < numCusts:
    ...
    results = pool.map(createSSNs, (
        numUniSSNs / 10, numUniSSNs / 10, numUniSSNs / 10, numUniSSNs / 10, numUniSSNs / 10, numUniSSNs / 10,
        numUniSSNs / 10, numUniSSNs / 10, numUniSSNs / 10, ((numUniSSNs / 10) + remainder)))
    liSSNMaster = results[0] + results[1] + results[2] + results[3] + results[4] + results[5] + results[6] + results[
        7] + results[8] + results[9]
    liSSNMaster = list(set(liSSNMaster))

# Multi-thread the customer list generation
pool2 = Pool(processes=proc)
remainder = numCusts % proc
custResults = pool2.map(create_customers, (
    numCusts / 10, numCusts / 10, numCusts / 10, numCusts / 10, numCusts / 10, numCusts / 10, numCusts / 10,
    numCusts / 10,
    numCusts / 10, ((numCusts / 10) + remainder)))
liCust = custResults[0] + custResults[1] + custResults[2] + custResults[3] + custResults[4] + custResults[5] + \
         custResults[6] + custResults[7] + custResults[8] + custResults[9]
